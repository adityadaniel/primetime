# Tech Canteen landing — `techcanteen.theprimetime.id`
...
| File | Purpose |
|------|---------|
| `index.html` | Landing page for `https://techcanteen.theprimetime.id` |
| `checkin.html` | Fallback page shown when a `/checkin?...` link is opened on a non-iOS device or a device without the App Clip yet |
| `styles.css` | Mirrors the PRIMETIME design tokens (vermilion, bone, ink, Big Shoulders Display, Newsreader) — kept identical to `../landing/styles.css` plus a `.stations` block |
...
This is a **separate Pages project** from the `theprimetime.id` apex landing.

- Build command: *(none — leave empty)*
- Build output directory: `landing-techcanteen`
- Production branch: `main`
- Custom domain: `techcanteen.theprimetime.id`

The `theprimetime.id` apex Pages project keeps using the existing `landing/` folder — Tech Canteen does not affect it.

### DNS

Add a CNAME on the `theprimetime.id` zone:
```
...
```
https://techcanteen.theprimetime.id/checkin?station=<id>
```
...
```
appclips:techcanteen.theprimetime.id
applinks:techcanteen.theprimetime.id
```
...
Once `https://techcanteen.theprimetime.id` is live:

```bash
# 1. Inspect what Apple's CDN sees
swcutil dl -d techcanteen.theprimetime.id

# 2. Or use Branch's web validator
# https://branch.io/resources/aasa-validator/

# 3. Manual check
curl -sI https://techcanteen.theprimetime.id/.well-known/apple-app-site-association
...
When the AASA needs to change (new bundle ID, new path component), edit `.well-known/apple-app-site-association`, push to `main`, Pages auto-deploys, then re-run `swcutil dl -d techcanteen.theprimetime.id` to confirm Apple's CDN refreshed (it caches AASA aggressively — can take up to 24 h).
