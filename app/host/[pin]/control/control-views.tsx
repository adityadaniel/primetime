      ...
      <section className="px-6 pt-6 max-w-[1500px] mx-auto grid grid-cols-12 gap-5">
        ...
        >
          ...
          <div className="mt-6 grid grid-cols-12 gap-4">
            ...
            />
            <div
              className="col-span-12 md:col-span-3 ink-border p-4 flex flex-col items-start"
              style={{ background: 'var(--bone)' }}
            >
          ...
          <div className="mt-8">
            {phase === 'lobby' && state && (
              <LobbyView state={state} onStart={onStart} onKick={onKick} />
            )}
            {(phase === 'question' || phase === 'reveal') && state && (
              <QuestionStage state={state} />
            )}
            {phase === 'leaderboard' && state && <LeaderboardStage state={state} />}
            {phase === 'final' && state && <FinalStage state={state} pin={pin} />}
          </div>
        </div>
        ...
        >
          ...
          <ol className="mt-3 divide-y" style={{ borderColor: 'rgba(15,15,15,.18)' }}>
            {board.length === 0 && (
              <li className="font-editorial italic opacity-60 py-3">No players have joined yet.</li>
            )}
            {board.map((p, i) => (
              <li
                key={p.id}
          ...
          <div
            className="mt-auto pt-4 border-t-2 flex flex-col gap-2"
...
function PanelStat({ cols, label, value }: { cols: string; label: string; value: string }) {
  return (
    <div className={`${cols} ink-border p-4 flex flex-col`} style={{ background: 'var(--bone)' }}>
            ...
            >
              ...
              <div className="relative flex items-center gap-3 p-3">
                ...
                <div className="flex-1">
                  <span
                    className="chyron opacity-70"
                    style={{ color: isCorrect ? 'var(--bone)' : undefined }}
                  >
                ...
                <div
                  className="ticker tabular-nums text-right"
                  style={{ color: isCorrect ? 'var(--bone)' : undefined }}
                >
