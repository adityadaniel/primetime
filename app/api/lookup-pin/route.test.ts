...
vi.mock('@/lib/db', () => ({
  prisma: {
    ...
    wonderWallSession: { findUnique: (args: unknown) => wwFindUnique(args) },
  },
}));
...
beforeEach(() => {
  ...
  wwFindUnique.mockReset().mockResolvedValue(null);
});
...
describe('POST /api/lookup-pin', () => {
  ...
  it('returns wonderwall with status for a WonderWall pin', async () => {
    wwFindUnique.mockResolvedValue({ id: 'wws_1', status: 'DRAFT' });
    const res = await POST(postReq({ pin: '123456' }));
    await expect(res.json()).resolves.toEqual({ type: 'wonderwall', status: 'DRAFT' });
  });
